...
-(void)setTableViewUp{
    ...
    [self.popBaseView addSubview:whiteView];
    CGFloat whiteViewH;
    CGFloat buttonH;
    ...
    [whiteView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.bottom.left.right.mas_equalTo(self.popBaseView);
        make.height.mas_equalTo(whiteViewH);
    }];
    ...
    if (!self.tableView) {
        self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0, self.popBaseView.bounds.size.width, self.popBaseView.bounds.size.height - buttonH - whiteViewH) style:UITableViewStylePlain];
        self.tableView.delegate = self;
        self.tableView.dataSource = self;
        [self.tableView registerNib:[UINib nibWithNibName:@"PlayerTableViewCell" bundle:nil] forCellReuseIdentifier:@"player"];
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    DetialModel *model = self.data[indexPath.row];
    PlayerTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"player"];
    if (!cell) {
        cell = [[PlayerTableViewCell alloc]initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"player"];
    }
    cell.playTitle.text = model.title;
    cell.playTime.text = model.duration;
    ...
    if (indexPath.row == self.defaultSelectRow) {
        ...
        cell.playTitle.textColor = [UIColor redColor];
    }else{
        cell.playTitle.textColor = [UIColor blackColor];
    }
